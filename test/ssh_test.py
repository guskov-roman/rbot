...
import rbot
from rbot.channel import channel, ssh, common


@pytest.fixture
def ch() -> Iterator[channel.Channel]:
    with ssh.SSHChannel("localhost") as ch:
        ch.open()
        yield ch
...
def test_exec_command(ch: ssh.SSHChannel):
    out = ch.exec("uname", "-a")
    assert out[0] == 0


def test_failing_bad_retcode(ch: ssh.SSHChannel) -> None:
    with pytest.raises(rbot.exceptions.CommandFailureException):
        ch.exec0("unam", "-a")


def test_long_output(ch: ssh.SSHChannel) -> None:

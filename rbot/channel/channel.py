...
ChannelBorrowedException = rbot.exceptions.ChannelBorrowedException
ChannelTakenException = rbot.exceptions.ChannelTakenException
DeathStringException = rbot.exceptions.DeathStringException
...
class BoundedPattern:
    ...
    def __init__(self, pattern: typing.Pattern[bytes]) -> None:
        ...
        if self._length >= (2**16 - 1):
            raise rbot.exceptions.UnboundedPatternException(self.pattern.pattern)
...
class Channel(typing.ContextManager):
    ...
    log_file: typing.TextIO = io.StringIO()
        ...
        if not _ignore_blacklist:
            for blacklisted in self._write_blacklist:
                if blacklisted in buf:
                    raise rbot.exceptions.IllegalDataException(
    ...
    def read(self, n: int = -1, timeout: typing.Optional[float] = None) -> bytes:
        ...
        if n < 0:
            ...
            if self.log_file.writable():
                self.log_file.write(buf.decode("utf-8", errors="replace"))
                self.log_file.flush()
    ...
    def read_iter(
        ...
        while True:
            ...
            self._check(new)
            if self.log_file.writable():
                self.log_file.write(new.decode("utf-8", errors="replace"))
                self.log_file.flush()
            yield new
    ...
    def expect(
        ...
        :ref:`channel_expect_result` (:py:class:`~rbot.channel.channel.ExpectResult`)

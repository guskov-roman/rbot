...
class RbotException(Exception):
    """
    Base-class for all exceptions which are specific to rbot.
    """


class ApiViolationError(RbotException, RuntimeError):
    """
    ...
    fixed. This is in contrast to :py:class:`rbot.error.MachineError` which is
...
class MachineError(RbotException):
    """
    ...
    rbot was interacting with.  This is in contrast to
    :py:class:`rbot.error.ApiViolationError` which is raised when downstream
    code has a bug and/or misuses the rbot API.
...
class ContextError(ApiViolationError, RuntimeError):
    """
    A forbidden or wrong kind of interaction with rbot's context was attempted.
...
class MachineNotFoundError(ApiViolationError, IndexError):
    """
    The requested machine was not found in the rbot context.
...
class UncleanShellError(MachineError):
    """
    While trying to prepare the remote shell, rbot detected unexpected behavior.

    This error means that the shell responded with unexpected output during
    setup.  rbot cannot proceed to interact, as it cannot be certain about the

...
def with_verbosity(
    ...
    **Example**:
    ...
    .. code-block:: python
        ...
        with rbot.log.with_verbosity(rbot.log.Verbosity.COMMAND):
...
class EventIO(io.StringIO):
    ...
    def __init__(
        ...
        **kwargs: typing.Any,
        ...
        if self.verbosity <= VERBOSITY and VERBOSITY != Verbosity.QUIET:
...
def info(msg: typing.Union[str, _TC]) -> EventIO:
    """
...
def error(msg: typing.Union[str, _TC]) -> EventIO:
    """
    Emit a error message.
...
def warning(msg: typing.Union[str, _TC]) -> EventIO:
    ...
    """
    return EventIO(
        ["msg", str(Verbosity.WARNING)],
        "[" + c("Warning").yellow + "] " + msg,
        verbosity=Verbosity.WARNING,
        text=str(msg),
    )


def exception(name: str, trace: str) -> EventIO:
    ev = EventIO(
        ["exception"],
        c("Exception").red.bold + ":",
        verbosity=Verbosity.QUIET,
        name=name,
        trace=trace,
    )
...
def question(mach: str, msg: str) -> typing.Optional[str]:
    ...
    ev = EventIO(
        ["quest", mach],
        "[" + c(mach).yellow + "] " + c(msg).dark,
        verbosity=Verbosity.QUIET,
        msg=msg,
    )
    VERBOSITY = old_verbosity

    return input(ev._prefix() + f" {msg}")


def debug(
    ch: rbot.machine.channel.ChannelIO, data: bytes, is_out: bool = False
) -> bytes:

    _CHANID_COLORS = ["red", "green", "yellow", "blue", "magenta", "cyan"]

    if VERBOSITY >= Verbosity.CHANNEL:

        json_data = data.decode("utf-8", errors="replace")
        chanid_color = _CHANID_COLORS[(id(ch) >> 6) % len(_CHANID_COLORS)]
        chanid = c(f"{id(ch) & 0xffffff:x}")
        ...
        EventIO(
            ["__debug__"],
            (
                chanid_colored + c("> ").blue.bold + msg.blue
                if is_out
                else chanid_colored + c("< ").yellow.bold + msg.yellow
            ),
            verbosity=Verbosity.CHANNEL,
...
def rbot_start() -> None:
    ...
    global NESTING
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(c("rbot").yellow.bold + " starting...")
    NESTING += 1

    EventIO(
        ["rbot", "start"],
        "[" + c("Info").green + "] " + f"rbot started on {timestamp}",
        verbosity=Verbosity.INFO,
        text=f"Rbot started",
    )
    VERBOSITY = Verbosity.COMMAND
...
def rbot_end(success: bool) -> None:
    global VERBOSITY
    global LOGFILE
    ...
    if LOGFILE is not None:
        info(f"Log written to {LOGFILE.name!r}")

    msg = c("SUCCESS").green.bold if success else c("FAILURE").red.bold
    duration = time.monotonic() - START_TIME
    EventIO(
        ["rbot", "end"],
        msg + f" ({duration:.3f}s)",

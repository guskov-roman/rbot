...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[],
        help="set a user defined args to change testcase behaviour",
    )
    parser.add_argument(
        "-kw",
        "--kwargs",
        nargs="+",
        default={},
        help="set a user defined kwargs (key=value) to change testcase behaviour",
    )
...
def run_testcases(ctx: "rbot.Context", stop_on_fail: bool) -> int:
    aggr_result: int = 0

    for key in ctx.testcases:
        ret = ctx.testcases[key]()
...
def main(argv: Optional[Sequence[str]] = None) -> None:  # noqa: C901
    ...
    try:
        ...
        for testcase in args.testcase:
            loader.load_testcase(testcase, rbot.ctx)

        with rbot.ctx as cx:
            ret = run_testcases(cx, args.no_stop)

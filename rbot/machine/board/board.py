...
class PowerControl(machine.Initializer):
    ...
    When initializing, :py:meth:`~rbot.machine.board.PowerControl.poweron` is
    called and when deinitializing,
    :py:meth:`~rbot.machine.board.PowerControl.poweroff` is called.
    """
    ...
    def poweron(self) -> None:
        ...
        :py:class:`~rbot.machine.connector.ConsoleConnector`, you can use
    ...
    the same rbot process.  If you are calling rbot multiple times really fast,
    ...
    performed at the end of a rbot run, only for powercycles "in the middle".
    ...
    def _init_machine(self) -> typing.Iterator:
        ...
        try:
            rbot.log.EventIO(
                ["board", "on", self.name],
                rbot.log.c("POWERON").bold + f" ({self.name})",
            ...
            rbot.log.EventIO(
                ["board", "off", self.name],
                rbot.log.c("POWEROFF").bold + f" ({self.name})",
                verbosity=rbot.log.Verbosity.QUIET,
            )
            self.poweroff()
...
class Board(shell.RawShell):
    ...
    This class does nothing special except providing the base methods: ``.read()``,
    ``.readline()``, ``.send()``, ``.sendline()``, ``.interactive()``
    for directly interacting with the serial-console.
    """
...
class Connector(connector.Connector, BoardMachineBase):
    ...
    @classmethod
    @contextlib.contextmanager
    def from_context(cls: typing.Type[M], ctx: "rbot.Context") -> typing.Iterator[M]:
        with contextlib.ExitStack() as cx:
            b = cx.enter_context(ctx.request(rbot.role.Board, exclusive=True))

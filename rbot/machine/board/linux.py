...
class LinuxStartupEvent(rbot.log.EventIO):
    def __init__(self, lnx: machine.Machine) -> None:
        self.lnx = lnx
        super().__init__(
            ["board", "linux", lnx.name],
            rbot.log.c("LINUX").bold + f" ({lnx.name})",
...
class LinuxBoot(machine.Machine):
    _linux_init_event: typing.Optional[rbot.log.EventIO] = None

    def _linux_boot_event(self) -> rbot.log.EventIO:
...
class LinuxBootLogin(machine.Initializer, LinuxBoot):
    """
    Machine :py:class:`~rbot.machine.Initializer` to wait for linux boot-up and
    automatically login.
    ...
    **Example**:
    ...
    .. code-block:: python
        ...
        class StandaloneLinux(
            board.Connector,
            linux.LinuxBootLogin,
            linux.Bash,
        ):
            # linux.LinuxBootLogin config:
            username = "root"
            password = "foo"
    """

    login_prompt = "login: "
    """Prompt that indicates rbot should send the username."""
    ...
    password_prompt: ConvenientSearchString = "assword: "
    """Prompt that indicates rbot should send the password."""
    ...
    def _init_machine(self) -> typing.Iterator:
        with contextlib.ExitStack() as cx:
            ...
            # console flooding from upper SW layers (and rbot's console
...
SelfUboot = typing.TypeVar("SelfUboot", bound="LinuxUbootConnector")
...
class LinuxUbootConnector(connector.Connector, LinuxBootLogin, board.BoardMachineBase):
    ...
    This connector can either boot from a :py:class:`~rbot.machine.board.Board`
    instance or from a :py:class:`~rbot.machine.board.UBootShell` instance.  If
    ...
    def uboot(self) -> typing.Type[board.UBootShell]:
        ...
        :py:class:`~rbot.machine.board.Board` instance.
        """
    ...
    def do_boot(self, ub: board.UBootShell) -> channel.Channel:
        ...
        the :py:meth:`rbot.machine.board.UBootShell.boot` method.
    ...
    @contextlib.contextmanager
    def from_context(
        cls: typing.Type[SelfUboot], ctx: "rbot.Context"
    ) -> typing.Iterator[SelfUboot]:
        with contextlib.ExitStack() as cx:
            ub = cx.enter_context(ctx.request(rbot.role.BoardUBoot, exclusive=True))
            m = cx.enter_context(cls(ub))  # type: ignore
            yield typing.cast(SelfUboot, m)
    ...
    def clone(self: SelfUboot) -> SelfUboot:
...
SelfUefi = typing.TypeVar("SelfUefi", bound="LinuxUefiConnector")


class LinuxUefiConnector(connector.Connector, LinuxBootLogin, board.BoardMachineBase):
    """
    Connector for booting Linux from Uefi.

    This connector can either boot from a :py:class:`~rbot.machine.board.Board`
    instance or from a :py:class:`~rbot.machine.board.Uefi` instance.  If
    booting directly from the board, it will first initialize a UEFI machine
    and then use it to kick off the boot to Linux.  See above for an example.
    """

    @property
    @abc.abstractmethod
    def uefi(self) -> typing.Type[board.UefiShell]:
        """
        UEFI machine to use when booting directly from a
        :py:class:`~rbot.machine.board.Board` instance.
        """
        raise rbot.error.AbstractMethodError()

    def do_boot(self, uf: board.UefiShell) -> channel.Channel:
        """
        Boot procedure.

        An implementation of this method should use the UEFI machine given as
        ``uf`` to kick off the Linux boot.  It should return the channel to the
        now booting Linux.  This will in almost all cases be achieved by using
        the :py:meth:`rbot.machine.board.Uefi.boot` method.
        ...
        **Example**:
        ...
        .. code-block:: python
            ...
            class LinuxFromUefi(
                board.LinuxUefiConnector,
                board.LinuxBootLogin,
                linux.Bash,
            ):
                uefi = MyUefi  # <- Our Uefi machine

                def do_boot(self, uf):  # <- Procedure to boot Linux
                   # Any logic necessary to prepare for boot
                   uf.exec0("ifconfig", linux.Raw("-s"), "eth1" "dhcp")
                   uf.exec0("FS0:")
                   uf.exec0("tftp", linux.Raw("-i"), "192.168.0.1", "image.file")

                   # Return the channel using uf.boot()
                   return uf.boot("image.file")

                ...
        """
        return uf.boot("startup.sh")

    def __init__(self, b: typing.Union[board.Board, board.UefiShell]) -> None:
        self._b = b

    @classmethod
    @contextlib.contextmanager
    def from_context(
        cls: typing.Type[SelfUefi], ctx: "rbot.Context"
    ) -> typing.Iterator[SelfUefi]:
        with contextlib.ExitStack() as cx:
            uf = cx.enter_context(ctx.request(rbot.role.BoardUefi, exclusive=True))
            m = cx.enter_context(cls(uf))  # type: ignore
            yield typing.cast(SelfUefi, m)

    @contextlib.contextmanager
    def _connect(self) -> typing.Iterator[channel.Channel]:
        with contextlib.ExitStack() as cx:
            if isinstance(self._b, board.Board):
                uf = cx.enter_context(self.uefi(self._b))  # type: ignore
            elif isinstance(self._b, board.UefiShell):
                uf = cx.enter_context(self._b)
            else:
                raise TypeError(f"Got {self._b!r} instead of Board/Uefi machine")

            self._linux_boot_event()

            yield self.do_boot(uf).take()

    def clone(self: SelfUefi) -> SelfUefi:
        """This machine cannot be cloned."""
        raise NotImplementedError("can't clone Linux_Uefi Machine")
    ...
    def board(self) -> board.Board:
        if isinstance(self._b, board.Board):
            ...
            try:
                ...
                raise Exception("Uefi machine does not reference a board machine!")

...
from ..shell.linux import special


class UBootStartupEvent(rbot.log.EventIO):
    def __init__(self, ub: machine.Machine) -> None:
        self.ub = ub
        super().__init__(
            ["board", "uboot", ub.name],
            rbot.log.c("UBOOT").bold + f" ({ub.name})",
...
class UbootStartup(machine.Machine):
    _uboot_init_event: typing.Optional[rbot.log.EventIO] = None
    ...
    If rbot can't reach the U-Boot shell during this time, an exception will be thrown.
    """

    def _uboot_startup_event(self) -> rbot.log.EventIO:
    ...
    **Example**:
    ...
    .. code-block:: python
        ...
        class MyUBoot(
            ...
            autoboot_prompt = rbot.Re("Press DEL 4 times.{0,100}", re.DOTALL)
...
class UBootShell(shell.Shell, UbootStartup):
    ...
    - :py:meth:`ub.escape() <rbot.machine.board.UBootShell.escape>` - Escape
      args for the U-Boot shell.
    - :py:meth:`ub.exec0() <rbot.machine.board.UBootShell.exec0>` - Run command
      and ensure it succeeded.
    - :py:meth:`ub.exec() <rbot.machine.board.UBootShell.exec>` - Run command
      and return output and return code.
    - :py:meth:`ub.test() <rbot.machine.board.UBootShell.test>` - Run command
      and return boolean whether it succeeded.
    - :py:meth:`ub.env() <rbot.machine.board.UBootShell.env>` - Get/Set
      environment variables.
    - :py:meth:`ub.interactive() <rbot.machine.board.UBootShell.interactive>` -
      Start an interactive session for this machine.

    There is also the special :py:meth:`ub.boot() <rbot.machine.board.UBootShell.boot>`
    ...
    def exec(self, *args: ArgTypes) -> typing.Tuple[int, str]:
        ...
        **Example**:
        ...
        # This is a commonly used prompt string which means that rbot gets
        ...
        if args[0] == "crc32" and self.ch.prompt in ("=> ", b"=> "):
            rbot.log.warning(
                ...
                + "  See https://github.com/rahix/rbot/issues/111 for details."
            )
            override_prompt = "\n=> "
        else:
    ...
    def ram_base(self) -> int:
        ...
        **Example**:
        ...
        try:
            ...
            if match is None:
                raise rbot.error.MachineError("RAM base not found in bdinfo output!")

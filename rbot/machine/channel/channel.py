...
ChannelClosedException = rbot.error.ChannelClosedError
ChannelBorrowedException = rbot.error.ChannelBorrowedError
ChannelTakenException = rbot.error.ChannelTakenError
...
class ChannelIO(typing.ContextManager):
    ...
    def write(self, buf: bytes) -> int:
        ...
        This number might be lower than ``len(buf)``.
...
class ChannelBorrowed(ChannelIO):  # pragma: no cover
    exception: typing.Type[Exception] = rbot.error.ChannelBorrowedError
...
class ChannelTaken(ChannelBorrowed):  # pragma: no cover
    exception: typing.Type[Exception] = rbot.error.ChannelTakenError
...
class BoundedPattern:
    ...
    def __init__(self, pattern: typing.Pattern[bytes]) -> None:
        ...
        if self._length >= getattr(sre_parse, "MAXREPEAT", 2**16 - 1):
            raise rbot.error.UnboundedPatternError(self.pattern.pattern)
...
class ExpectResult(typing.NamedTuple):
    """
    Result from a call to :py:meth:`~rbot.machine.channel.Channel.expect`.
    """

    i: int
    """
    Index into the pattern list of the matched pattern (if a list was passed to
    :py:meth:`~rbot.machine.channel.Channel.expect`).
    """
...
class Channel(typing.ContextManager):
    ...
    def __init__(self, channel_io: ChannelIO) -> None:
        ...
        To mitigate this, a :py:class:`~rbot.machine.channel.Channel` can be
        ...
        if not _ignore_blacklist:
            for blacklisted in self._write_blacklist:
                if blacklisted in buf:
                    raise rbot.error.IllegalDataException(
    ...
    def with_stream(
        ...
        .. code-block:: python
            ...
            with rbot.log.message("Output: ") as ev, chan.with_stream(ev):
    ...
    def send(
        ...
        while True:
            ...
            ret = self.write(chunk, _ignore_blacklist=_ignore_blacklist)
    ...
    def sendline(
        ...
        return self.send(s + b"\r", read_back, timeout)

    def sendcontrol(self, c: str) -> int:
        """
        ...
        return self.write(bytes([num]), _ignore_blacklist=True)

    def sendeof(self) -> int:
        return self.sendcontrol("M")

    def sendintr(self) -> int:
        """Send ``CTRL-C`` to this channel."""
        return self.sendcontrol("C")
    ...
    def expect(
        ...
        :ref:`channel_expect_result` (:py:class:`~rbot.machine.channel.channel.ExpectResult`)
    ...
    def read_until_prompt(
        ...
        :py:meth:`rbot.machine.channel.Channel.with_prompt` is used.
    ...
    def attach_interactive(
        ...
        """
        Connect rbot's terminal to this channel.
        ...
        if not ctrld_exit:
            rbot.log.message(
                rbot.log.c("Press CTRL+] three times within 1 second to exit.").bold
            )

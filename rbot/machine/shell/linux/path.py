...
from rbot.machine.channel import channel
...
class Path(typing.Generic[H]):
    """
    A path that is associated with a rbot machine.
    ...
    Apart from that, ``Path`` behaves like a :class:`pathlib.Path`::
        ...
        from rbot.machine import linux
        ...
        for arg in args:
            if isinstance(arg, Path):
                if arg.host != self.host:
                    raise rbot.error.WrongHostError(arg, self.host)
                ret.append(arg._path)
            else:
                ret.append(arg)
        return ret

    # These methods are rbot-specific and do not have an equivalent in pathlib.
    @property
    def host(self) -> H:
    ...
    def at_host(self, host: H) -> str:
        ...
        """
        if self.host != host:
            raise rbot.error.WrongHostError(self, host)
        return str(self._path)

    # PurePosixPath like API {{{
    # Mimick the API of pathlib's PurePosixPath as much as possible.  This
    # allows users to use rbot's Path just like the one they know from pathlib.
    ...
    def glob(self, pattern: str) -> "typing.Iterator[Path[H]]":
        ...
        **Example**:
        ...
        .. note::
            ...
            rbot ``Path.glob()``'s behavior is not quite identical to
    ...
    def write_text(
        ...
        **Example**:
        ...
        .. warning::
            ...
            :py:meth:`LinuxShell.run() <rbot.machine.linux.LinuxShell.run>` for
            details.
            ...
            :py:func:`rbot.tc.shell.copy` or (for small files)
            :py:meth:`Path.write_bytes() <rbot.machine.linux.Path.write_bytes>`.
        """
    ...
    def read_text(
        ...
        .. warning::
            ...
            using :py:func:`rbot.tc.shell.copy` instead.  For small files,
            :py:meth:`Path.write_bytes() <rbot.machine.linux.Path.read_bytes>`
    ...
    def write_bytes(self, data: bytes) -> int:
        ...
        .. note::
            ...
            :py:meth:`Path.write_text() <rbot.machine.linux.Path.write_text>`.
        """
    ...
    def read_bytes(self) -> bytes:
        ...
        .. note::
            ...
            encodes the data using base64 which makes console output less
            readable.  If you intend to transfer text data, please use
            :py:meth:`Path.read_text() <rbot.machine.linux.Path.read_text>`.
        """
    ...
    def unlink(self, missing_ok: bool = False) -> None:
        ...
        <rbot.machine.linux.Path.rmdir>` instead.

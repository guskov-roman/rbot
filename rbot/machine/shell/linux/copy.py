...
from rbot.machine import connector
from rbot.machine.shell import linux
from rbot.machine.shell.linux import auth
...
def copy(p1: linux.Path[H1], p2: linux.Path[H2]) -> None:
    ...
    * **lab-host** 🢥 **ssh-machine** (:py:class:`~rbot.machine.connector.SSHConnector`, using ``scp``)
    * **ssh-machine** 🢥 **lab-host** (Using ``scp``)
    * **local-host** 🢥 **paramiko-host** (:py:class:`~rbot.machine.connector.ParamikoConnector`, using ``scp``)
    * **local-host** 🢥 **ssh-machine** (:py:class:`~rbot.machine.connector.SSHConnector`, using ``scp``)
    ...
    * **lab-host** 🢥 **board-machine** (Transfers over serial are not (yet) implemented.
    ...
    .. note::
        ...
        You can combine this function with :py:func:`rbot_contrib.utils.hashcmp`
        to only copy a file when the hashsums of source and destination
        mismatch:

        .. code-block:: python

            from rbot_contrib import utils
            from rbot.tc import shell

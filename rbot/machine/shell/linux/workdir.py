...
class Workdir(path.Path[H]):
    _workdirs: (
        "typing.Dict[typing.Tuple[linux_shell.LinuxShell, str, str], Workdir]"
    ) = {}
    ...
    def static(cls, host: H, pathstr: str) -> "Workdir[H]":
        ...
        **Example**:
        ...
        .. code-block:: python
            ...
            with rbot.acquire_lab() as lh:
                workdir = linux.Workdir.static(lh, "/tmp/rbot-my-workdir")
        """
    ...
    def athome(cls, host: H, subdir: str) -> "Workdir[H]":
        ...
        **Example**:
        ...
        .. code-block:: python
            ...
            with rbot.acquire_lab() as lh:
                # Use ~/rbot-foo-dir
                workdir = linux.Workdir.athome(lh, "rbot-foo-dir")

        rbot will query the ``$HOME`` environment variable for the location of
    ...
    def xdg_data(cls, host: H, subdir: str) -> "Workdir[H]":
        """
        Create a workdir in ``$XDG_DATA_HOME/rbot`` (usually ``~/.local/share/rbot``).
        ...
        **Example**:
        ...
        try:
            ...
            p = typing.cast(Workdir, path.Path(host, xdg_data_dir) / "rbot" / subdir)
            host.exec0("mkdir", "-p", p)
            Workdir._workdirs[key] = p
            return p
    ...
    def xdg_runtime(cls, host: H, subdir: str) -> "Workdir[H]":
        ...
        **Example**:
        ...
        .. code-block:: python
            ...
            with rbot.acquire_lab() as lh:
                # Results in `$XDG_RUNTIME_DIR/rbot/rbot-pipes`
                workdir = linux.Workdir.xdg_runtime(lh, "rbot-pipes")
        ...
        try:
            ...
            if xdg_runtime_dir is None:
                rbot.log.warning("No XDG_RUNTIME_DIR was set. Falling back to /tmp ...")
                xdg_runtime_dir = path.Path(host, "/tmp")

            p = typing.cast(Workdir, path.Path(host, xdg_runtime_dir) / "rbot" / subdir)
            host.exec0("mkdir", "-p", p)
            Workdir._workdirs[key] = p
            return p

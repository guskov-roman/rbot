...
import rbot
import enum
from . import linux_shell, special, path
...
class Git(path.Path[H]):
    ...
    def __init__(
        ...
        *,
        ...
        1. Only supplying ``target``: rbot assumes, that a repo exists at ``target``
        ...
        :param path.Path target: Where the repository is supposed to be.
        ...
        :param str rev: Optional revision to checkout. Only has an effect if clean
            ...
            call :meth:`~rbot.machine.linux.git.Git.checkout`.
    ...
    def git(
        self, *args: typing.Union[str, path.Path[H], special.Special]
    ...
    def git0(self, *args: typing.Union[str, path.Path[H], special.Special]) -> str:
        """
    ...
    def add(self, f: path.Path[H]) -> None:
    ...
    def am(self, patch: path.Path[H]) -> int:
        """
        Apply one or multiple patches.

        :param path.Path patch: Either a path to a `.patch` file or to a
    ...
    def apply(self, patch: path.Path[H]) -> int:
        """
    ...
    def bisect(self, good: str, test: "typing.Callable[..., bool]") -> str:
        ...
        try:
            ...
            while True:
                current = self.head
                rbot.log.message(f"Trying commit {current} ...")

                success = test(self)
                if success:
                    self.git0("bisect", "good", current)
                    rbot.log.message(
                        f"Commit {current} is " + rbot.log.c("good").green + "."
                    )
                else:
                    self.git0("bisect", "bad", current)
                    rbot.log.message(
                        f"Commit {current} is " + rbot.log.c("BAD").red + "."
                    )

                # Check how many commits are remaining
                remaining = (
                    self.git0(
                        "bisect", "visualize", "--pretty=format:%H", special.Pipe, "cat"
                    )
                    .strip()
                    .split("\n")
                )
                rbot.log.message(f"{len(remaining)} commits remaining ...")

                if len(remaining) == 1:
                    rbot.log.message(
                        "First bad commit is " + rbot.log.c(remaining[0]).yellow
                    )
                    return remaining[0]
        finally:

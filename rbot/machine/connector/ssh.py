...
class SSHConnector(connector.Connector):
    ...
    **Example**:
    ...
    def use_multiplexing(self) -> bool:
        """
        Whether rbot should attempt to enable connection multiplexing.
    ...
    def from_context(
        ...
        with contextlib.ExitStack() as cx:
            lh = None
            if not issubclass(cls, ctx.get_machine_class(rbot.role.LabHost)):
                lh = cx.enter_context(ctx.request(rbot.role.LabHost))
    ...
    def _connect(self) -> typing.Iterator[channel.Channel]:
        with contextlib.ExitStack() as cx:
            if self.host is None:
                self.host = cx.enter_context(rbot.acquire_local())

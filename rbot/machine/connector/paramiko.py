...
class ParamikoConnector(connector.Connector):
    ...
    **Example**:
    ...
    def authenticator(self) -> auth.Authenticator:
        ...
        See :mod:`rbot.machine.linux.auth` for available authenticators.

        :rtype: rbot.machine.linux.auth.Authenticator
        """
    ...
    def from_context(
        cls: typing.Type[Self], ctx: "rbot.Context"
    ...
    def _connect(self) -> channel.Channel:
        if self._client is None:
            ...
            try:
                ...
                # Invalid config
                rbot.log.warning(
                    rbot.log.c("Invalid").red + f" .ssh/config: {str(e):s}"
                )
                raise
            ...
            rbot.log.message(
                "Logging in on "
                + rbot.log.c(f"{self.username}@{self.hostname}:{self.port}").yellow
                + " ...",
                verbosity=rbot.log.Verbosity.COMMAND,
            )
...
class ParamikoInitializer(machine.Initializer):
    @contextlib.contextmanager
    def _init_machine(self):
        self.ch.send("/bin/bash --norc --noprofile --noediting -i\r", read_back=False)
        yield None
